...
void c_ragebot::aim(c_cs_player* local, c_user_cmd* cmd, bool& send_packet)
{
	...
	client_entity_list()->for_each_player([&] (c_cs_player* player) -> void
	{
		...
		const auto firing = animation_system->get_latest_firing_animation(player);
		const auto up_pitch = animation_system->get_latest_up_pitch_animation(player);
		const auto sideways = animation_system->get_latest_sideways_animation(player);
		...
		*/
		...
		if (firing.has_value()) {
			...
			if (alternative.has_value()) {
				if (!target.has_value() || (alternative.value().damage > target.value().damage))
					target = alternative;
			}
		}
		...
		else if (up_pitch.has_value()) {
			const auto alternative = scan_record(local, up_pitch.value());//lookingup
			if (alternative.has_value()) {
				if (!target.has_value() || (alternative.value().damage > target.value().damage))
					target = alternative;
			}
		...
		else if (!target.has_value() && uncrouched.has_value()) {//use this only if we have nothing better lol, i suppose this is against fakeduck?
		...
		else if (!target.has_value()) {//we are running out of ideas, try some normal stuff
			const auto alternative = scan_record(local, latest.value());//latest
			if (alternative.has_value() &&
				(!target.has_value() || target.value().damage < alternative.value().damage))
				target = alternative;
		}
		else if (!target.has_value() && oldest.has_value() && latest.value() != oldest.value() /*&& oldest.value()->velocity.length2d() >= .1f*/) {//no need for velocity check he could have changed angles
			const auto alternative = scan_record(local, oldest.value());
			// is there no other record?
			if ((alternative.has_value() && !target.has_value()) ||
				(alternative.has_value() && target.has_value() && alternative.value().damage > target.value().damage))
				target = alternative;
		}

		/*const auto alternative = scan_record(local, latest.value());
		
		if (!target.has_value() || (target.has_value()
			&& target.value().damage < target.value().damage))
			target = alternative;
	...
	if ((wpn_info->get_weapon_id() == WEAPON_G3SG1 || wpn_info->get_weapon_id() == WEAPON_SCAR20
		...
		|| wpn_info->get_weapon_id() == WEAPON_SG553 || wpn_info->get_weapon_id() == WEAPON_AUG) && weapon->get_zoom_level() == 0)
	...
	if (!best_match.alt_attack)
	{
		...
		shot.uppitch = best_match.animation->up_pitch;
...
void c_ragebot::autostop(c_cs_player* local, c_user_cmd* cmd)
{
	...
	if (nospread->get_int() || !local->is_on_ground() ||
		(weapon && weapon->get_item_definition() == WEAPON_ZEUS) && local->is_on_ground())
		return;
...
void c_ragebot::fake_crouch(c_user_cmd* cmd)
{
	int fakelag_limit = antiaim->get_max_choke_amount();
	int choked_goal = fakelag_limit / 2;
	bool should_crouch = client_state->choked_commands >= choked_goal;

	if (GetAsyncKeyState(config.rage.fake_duck))
	{
		if (should_crouch)
			cmd->buttons |= c_user_cmd::duck;
		else
			cmd->buttons &= ~c_user_cmd::duck;
	}
}
...
std::optional<c_ragebot::aim_info> c_ragebot::scan_record(c_cs_player* local, c_animation_system::animation* animation)
{
	...
	const auto is_zeus = weapon->get_item_definition() == WEAPON_ZEUS;
...
std::optional<c_ragebot::aim_info> c_ragebot::scan_record_gun(c_cs_player* local, c_animation_system::animation* animation, std::optional<c_vector3d> pos)
{
	...
	if (!should_baim)
	{
		if (data->get_weapon_id() == WEAPON_AWP)
			should_baim = true;

		//if (animation->player->get_health() <= cfg.body_aim_health)
		//	should_baim = true;

		//baim if under 50HP + lethal
		if (animation->player->get_health() <= 50 && animation->player->get_health() <= data->iDamage)
			should_baim = true;
		...
		//if lethal
		//if (animation->player->get_health() < data->iDamage)
		//	should_baim = true;
		...
		//after missed shots
		if ( (info->missed_due_to_resolver + info->missed_due_to_spread) >= 2)
			should_baim = true;
	...
	auto should_try_head_cuz_shot = animation->didshot;
	auto should_try_head_cuz_side = animation->sideways;

	if ( (should_try_head_cuz_shot || should_try_head_cuz_side))
	{
		if (hitboxes_scan.size() > 0)
			hitboxes_scan.clear();

		if (should_baim == false)
		{
	...
	else if (data->get_weapon_id() == WEAPON_G3SG1 || data->get_weapon_id() == WEAPON_SCAR20)
	{
		if (hitboxes_scan.size() > 0)
			hitboxes_scan.clear();
	...
	/*
		...
		for (const auto& hitbox : c_cs_player::hitboxes_auto)
			scan_box(hitbox);
	else
	...
	*/
	...
	should_baim = false; //just in case
	return std::nullopt;
}
