...
void c_damage_esp::draw_damage()
{
	auto local = c_cs_player::get_local_player();

	if (!local || !local->is_alive() || !config.esp.hitmarker)
		return;
	...
	for (int i = 0; i < MAX_FLOATING_TEXTS; i++)
	{
		...
		if (txt->damage >= 90 && txt->damage <= 800)
			...
			damage_color = c_color(220, 155, 0, 255);
		else if (txt->damage >= 0 && txt->damage <= 49)
			damage_color = c_color(0, 255, 0, 255);

		renderer->text(origin_screen - c_vector2d(0, 18), msg, damage_color, fnv1a("pro12"));
	}
}

void c_damage_esp::on_bullet_impact(c_game_event* event)
{
	auto local = c_cs_player::get_local_player();
...
void c_damage_esp::on_player_hurt(c_game_event* event)
{
	auto local = c_cs_player::get_local_player();
