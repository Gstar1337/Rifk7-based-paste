...
std::vector<std::string> g_clantag_table = {
	"$ ",
	"$ f",
	"$ fl",
	"$ fle",
	...
	if (local && engine_client()->is_ingame()) {
		...
		if (iCurrentTime != iLastTime) {
			...
			if (config.rage.experimental_aa) {
				auto tag = g_clantag_table[int(global_vars_base->curtime) % g_clantag_table.size()];

				ChangeClantag(tag, "$ flexware");
			}
			else
				ChangeClantag("", "not so flex");
		}
