...
float c_antiaim::get_feet_yaw(c_cs_player* local)
{
	if (!local)
		return 0.f;
	...
	if (current_feet_yaw >= -360)
		current_feet_yaw = std::min(current_feet_yaw, 360.f);
...
float c_antiaim::at_target() {
	...
	c_qangle best_angles = c_vector3d(0.0f, 0.f, 0.0f);
	...
	client_entity_list()->for_each_player([&](c_cs_player* player) -> void 
	{
		...
		{
			best_angles = c_vector3d(0.0f, 0.f, 0.0f);
			return;
		}

		c_vector3d local_position = local->get_abs_origin();
		c_vector3d target_position = player->get_abs_origin();

		c_vector3d viewangle = engine_client()->get_view_angles();

		const float fov = math::get_fov(viewangle, local->get_origin(), player->get_origin());
		if (fov < best_fov) {
			best_fov = fov;
			best_angles = math::to_angle(target_position - local_position);

			if (!math::normalize_yaw(best_angles.y))
			{
				best_angles = c_vector3d(0.0f, 0.f, 0.0f);
				return;
			}
...
void c_antiaim::fakelag(c_cs_player* local, c_user_cmd* cmd, bool& send_packet)
{
	...
	static constexpr auto target_standing = 6;
	static constexpr auto target_moving = 14;
	static constexpr auto target_air = 14;
	...
	max_choke_amount = game_rules->is_valve_ds() ? 6u : 14u;
	...
	{
		fakeland = fabsf(math::normalize_yaw(get_feet_yaw(local)
	...
	if (global_vars_base->curtime + global_vars_base->interval_per_tick > next_lby_update
		...
		&& anim_state->velocity.length2d() < 0.f)
...
void c_antiaim::run(c_cs_player* local, c_user_cmd* cmd, bool& send_packet)
{
	if (!local || !cmd || !input)
		return;
	...
	*/
	...
	else
	{
		...
		else
		{
			...
			if (!local->get_velocity().length2d() > 0.1 && local->is_on_ground()) //standing
				cmd->viewangles.y = last_fake = math::normalize_yaw(get_feet_yaw(local) + RandomFloat(target_delta, -target_delta));
			else																  //moving
				cmd->viewangles.y = last_fake = math::normalize_yaw(alternate ? get_feet_yaw(local) + 50.f : get_feet_yaw(local) - 50.f);
...
void c_antiaim::predict(c_cs_player* local, c_user_cmd* cmd)
{
	...
	const auto time = global_vars_base->curtime;//ticks_to_time(local->get_tick_base());
...
float c_antiaim::get_max_choke_amount()
{
	return max_choke_amount;
}
