...
void c_indicators::draw_radio_info(c_vector2d& position)
{
	if (strlen(BASS::bass_metadata) > 0 && config.misc.radio_channel)
	{
		...
		renderer->text(position, radio, c_color::primary(), fnv1a("pro12"), c_font::font_flags::drop_shadow);
		position.y += 16;

		renderer->text(position, BASS::bass_metadata, c_color::foreground(), fnv1a("pro12"), c_font::font_flags::drop_shadow);
		position.y += 16;

		if (radio_muted)
		{
			_rt(rad_muted, muted);
			renderer->text(position, rad_muted, c_color::gradient3(), fnv1a("pro12"), c_font::font_flags::drop_shadow);
		}
	}
...
void c_indicators::draw_velocity(c_vector2d& position)
{
	...
	//static auto maxvel = cvar()->find_var("sv_maxvelocity")->get_float();
	...
	static const auto velocity = __("Velocity");
	_rt(vel, velocity);
	draw_progressbar(position, vel, c_color::gradient4(), c_color::gradient1(), value);
}

void c_indicators::draw_fakeduck(c_vector2d& position)
{
	if (!config.rage.enabled)
		return;
	...
	static const auto fakeduck = __("Fakeduck");
	_rt(fd, fakeduck);
	draw_progressbar(position, fd, c_color::gradient3(), c_color::primary(), value);
}
...
{
	renderer->text(position, name, c_color::foreground(), fnv1a("pro12"), c_font::font_flags::drop_shadow);
	position.y += 16;
