...
void c_net_channel_::update_incoming_sequences(c_net_channel* channel) {
	static int lastseq = -1;
	//if (lastseq == channel->in_sequence_nr)return;

	c_net_channel_::sequences.push_front(c_incoming_sequence(channel->in_reliable_state, 0, 0, channel->in_sequence_nr, global_vars_base->curtime));
...
int c_net_channel_::send_datagram(c_net_channel* channel, uint32_t, void* buffer)
{
	...
	update_incoming_sequences(channel);
